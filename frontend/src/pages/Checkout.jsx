...
const Checkout = () => {
  ...
  const [formData, setFormData] = useState({
    receiverName: "", phone: "", street: "", note: "",
  ...
  const [discountInfo, setDiscountInfo] = useState({ amount: 0, message: "", isApplied: false });
  ...
  const validateForm = () => {
    let newErrors = {};
    if (!formData.receiverName.trim()) newErrors.receiverName = "Nhập tên người nhận";
  ...
  if (loading) return <div className="p-10 text-center font-semibold text-gray-600">Đang tải dữ liệu...</div>;
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            {!showAddressForm && selectedAddress ? (
              <div className="border border-blue-200 bg-blue-50/30 p-4 rounded-lg relative">
                <div className="flex items-center gap-4 mb-2">
                  <span className="font-bold text-gray-800 text-lg">{selectedAddress.receiverName}</span>
                  <span className="text-gray-500">|</span>
                  <span className="font-bold text-gray-800">{selectedAddress.phone}</span>
                  {selectedAddress.isDefault && (
                    <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded border border-blue-200">Mặc định</span>
                  )}
                </div>
              ...
              <div className="space-y-4 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <input type="text" placeholder="Họ tên người nhận" value={formData.receiverName}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <select value={formData.provinceCode} onChange={handleProvinceChange} className={`w-full border p-2.5 rounded-md outline-none ${errors.provinceCode ? 'border-red-500' : ''}`}>
                  ...
                  <div>
                    <select value={formData.districtCode} onChange={handleDistrictChange} disabled={!formData.provinceCode} className={`w-full border p-2.5 rounded-md outline-none ${errors.districtCode ? 'border-red-500' : ''}`}>
                  ...
                  <div>
                    <select value={formData.wardCode} onChange={(e) => setFormData({...formData, wardCode: e.target.value, wardName: e.target.options[e.target.selectedIndex].text})} disabled={!formData.districtCode} className={`w-full border p-2.5 rounded-md outline-none ${errors.wardCode ? 'border-red-500' : ''}`}>
                      <option value="">Phường/Xã</option>
                      {wards.map(w => <option key={w.code} value={w.code}>{w.name}</option>)}
                    </select>
                    {errors.wardCode && <p className="text-red-500 text-xs mt-1">{errors.wardCode}</p>}
                  </div>
                </div>

                <div>
                  <input type="text" placeholder="Số nhà, tên đường..." value={formData.street}
            ...
            <div className="mt-4">
              <textarea rows="2" placeholder="Ghi chú đơn hàng (Tùy chọn)" value={formData.note}
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            <div className="flex gap-4">
              ...
              >
                <span className="text-2xl">📱</span>
                <span className="font-semibold text-gray-700">Chuyển khoản / QR Pay</span>
              </button>
            </div>
              ...
              <div className="mt-4 p-5 bg-gray-50 border border-gray-200 rounded-xl transition-all duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  >
                    ...
                    <img src="https://cdn.haitrieu.com/wp-content/uploads/2022/10/Logo-VietQR.png" alt="VietQR" className="h-8 object-contain" />
                  ...
                  >
                    ...
                    <img src="https://upload.wikimedia.org/wikipedia/vi/f/fe/MoMo_Logo.png" alt="MoMo" className="h-8 object-contain" />
                  ...
                  >
                    ...
                    <span className="text-xs font-semibold text-gray-700 text-center">VNPAY-QR</span>
                  </div>
                </div>
                ...
                <p className="mt-4 text-xs text-gray-500 italic flex items-center gap-1.5">
                  ...
                  Mã QR thanh toán sẽ được tạo tự động ngay sau khi bạn bấm đặt hàng.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 sticky top-6">
            ...
            {cartData.items.length === 0 ? (
              ...
              <div className="space-y-4 mb-4 border-b pb-6 max-h-[300px] overflow-y-auto pr-2">
                {cartData.items.map((item) => (
                  <div key={item._id || item.product_id?._id} className="flex justify-between items-start gap-2">
                    <div className="flex gap-3">
                      <img src={item.product_id?.imageUrl || "https://dummyimage.com/60x60"} alt="Product" className="w-14 h-14 object-cover rounded-md border" />
                      <div>
                        <p className="font-medium text-gray-800 text-sm line-clamp-2 leading-tight">{item.product_id?.name || "Sản phẩm"}</p>
                    ...
                    <p className="font-semibold text-gray-800 text-sm whitespace-nowrap">
                      {item.product_id?.price ? (item.product_id.price * item.quantity).toLocaleString() : 0}đ
                    </p>
                  </div>
                ))}
              </div>
            ...
            <div className="py-4 border-b mb-4">
              <label className="text-sm font-semibold text-gray-700 mb-2 block">Mã giảm giá</label>
            ...
            <div className="space-y-3 mb-6 text-sm">
              ...
              {cartData.discountAmount > 0 && (
                <div className="flex flex-col gap-1 text-green-600">
                  ...
                  {cartData.discountMessages?.map((msg, idx) => (
                    <span key={idx} className="text-xs italic pl-2">🎁 {msg}</span>
                  ))}
                </div>
              )}
              ...
              <div className="flex justify-between items-center pt-4 border-t">
                ...
                <span className="text-red-600 text-2xl font-bold">
                  {cartData.finalPrice ? cartData.finalPrice.toLocaleString() : 0}đ
                </span>
              </div>
