...
const UserManager = () => {
  ...
  const getRoleBadge = (role) => {
    if (role === 'admin') {
      return <span className="flex items-center gap-1 w-fit bg-red-50 text-red-700 border border-red-200 px-2 py-1 rounded text-xs font-bold uppercase"><Shield size={12}/> Admin</span>;
    }
    return <span className="flex items-center gap-1 w-fit bg-green-50 text-green-700 border border-green-200 px-2 py-1 rounded text-xs font-bold uppercase"><UserIcon size={12}/> Customer</span>;
  };
  ...
  const getTierBadge = (tier) => {
    switch (tier) {
      ...
      case 'silver':
        return <span className="flex items-center gap-1 w-fit bg-gray-200 text-gray-800 border border-gray-300 px-2 py-1 rounded text-xs font-bold uppercase"><Crown size={12}/> Silver</span>;
      case 'normal':
    ...
    <div className="p-6 md:p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800 uppercase tracking-wide">Quản lý Tài Khoản</h1>
            <p className="text-gray-500 text-sm mt-1">Tổng số người dùng: {users.length}</p>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 text-sm">
                {loading ? (
                  <tr><td colSpan="7" className="p-10 text-center text-gray-500 font-medium">Đang tải dữ liệu...</td></tr>
                ) : currentUsers.length === 0 ? (
                  <tr><td colSpan="7" className="p-10 text-center text-gray-500">Không tìm thấy người dùng nào.</td></tr>
                ) : (
                  currentUsers.map((user) => (
                    <tr key={user._id} className="hover:bg-gray-50 transition-colors">
                      <td className="p-4 font-mono text-xs text-gray-500">{user._id.slice(-6).toUpperCase()}</td>
          ...
          {totalPages > 1 && (
            <div className="p-4 border-t flex items-center justify-between bg-gray-50">
              ...
              <div className="flex gap-2">
                <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="p-2 border bg-white rounded-md hover:bg-gray-100 disabled:opacity-50 transition"><ChevronLeft size={18} /></button>
                <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="p-2 border bg-white rounded-md hover:bg-gray-100 disabled:opacity-50 transition"><ChevronRight size={18} /></button>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-xl w-full max-w-md shadow-2xl overflow-hidden">
              <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <h2 className="text-lg font-bold uppercase text-gray-800">Thêm Người Dùng Mới</h2>
              ...
              <form onSubmit={handleAddUser} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <select value={newUser.role} onChange={(e) => setNewUser({...newUser, role: e.target.value})} className="w-full p-2 border rounded-lg outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                ...
                <div className="pt-4 flex gap-3">
                  <button type="button" onClick={() => setIsAddModalOpen(false)} className="flex-1 py-2 bg-gray-200 text-gray-800 font-semibold rounded-lg hover:bg-gray-300 transition">Hủy</button>
